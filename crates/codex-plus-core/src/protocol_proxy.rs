...
pub async fn open_responses_proxy_request_for_path_and_originator(
    body: &str,
    original_user_agent: Option<&str>,
    originator: Option<&str>,
    request_path: &str,
...
pub async fn open_models_proxy_request(
    ...
    let request = if is_anthropic {
        ...
        with_relay_auth(client.get(&endpoint), &relay)
    };
...
pub async fn open_audio_transcriptions_proxy_request(
    ...
    let request = crate::http_client::proxied_client(&protocol_proxy_original_first_user_agent(
        &relay.user_agent,
        original_user_agent,
    ))?
    .post(endpoint)
...
pub async fn open_chat_completions_proxy_request(
    ...
    let is_stream = request_json
        ...
        .and_then(Value::as_bool)
        .unwrap_or(false);
    let request = crate::http_client::proxied_client(
...
async fn upstream_request_parts(
    ...
    if compact && relay.protocol != RelayProtocol::Responses {
        anyhow::bail!("仅 Responses 协议支持 Responses compact 请求");
    }
    let (url, mut body, wire_api) = match relay.protocol {
        RelayProtocol::Responses => {
            ...
            (url, body, UpstreamWireApi::Responses)
        }
    ...
    if guard_inline_image_data_urls(&mut body) {
        let _ = crate::diagnostic_log::append_diagnostic_log(
            ...
            json!({
                "model": body.get("model").and_then(Value::as_str).unwrap_or(""),
                "protocol": format!("{:?}", relay.protocol),
            }),
        );
        debug_assert!(false, "base64 图片泄漏进文本字段，检查协议转换路径");
    }
    Ok((url, body, wire_api))
}
